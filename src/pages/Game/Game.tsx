...
const generateGameState = (rows: number, columns: number) => Array(rows).fill(Array(columns).fill(0));
    ...
    return (
        <div style = {{ display: 'flex', justifyContent: 'space-around'}}>
            <S.ControlPanel>
                ...
                <button onClick = { () => setGameState(generateGameState(ROWS, COLUMNS)) }>apply changes OR CRASH</button>
                <button onClick = { () => {
                    setRunning(!running);
                    if (!running) {
